...
Long_t covfefe::hist_clust(){
  ...
  h1Pkpi2=new TH1D("GapP","stat",300,0.11,.26);
  h1Pgap=new TH1D("Momentum","stat",160,100.,260.);
  h1Nkmu2=new TH1D("Nkmu2","stats",3,-.5,2.5);
  ...
  std::string tf1Sname[]={"tof1SNoCut","tof1SKpi2Cut","tof1Stof1Cut","tof1SAngCut"};
...
Long_t covfefe::process_clust(){
  if(clsmar->fVertSP>0.0){
    // Fill good gap momentum:
    h1Pgap->Fill(clsmar->fVertSP);
    // select number of kmu2 candidates from P_kmu2 distribution: Fill register
    if(clsmar->fVertSP>=230.172 &&clsmar->fVertSP<=236.182) h1Nkmu2->Fill(1);
    ...
    if(clsmar->clusterM==2 && clsmar->E_prim2>0.045){
      ...
      if(clsmar->fVertSP>=195. && clsmar->fVertSP<=205.){
